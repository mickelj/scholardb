...
function searchWorks(req, res, next) {
  ...
  if (query) {
    db.run("SELECT work_id, work_data->>'title' as title, TS_RANK_CD(full_title_search, query, 32/* rank/(rank+1) */) AS rank FROM works_new, TO_TSQUERY('''" + query + "''') query WHERE query @@ full_title_search ORDER BY rank DESC", function(err, results) {
      if (err) {
        return next(err);
      }
